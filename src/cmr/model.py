...
from src.cmr.logic import ConceptEmbedding, reasoning
from src.cmr.rule_module import ProbRDCat
...
class CMRModel(pl.LightningModule):
    ...
    def calc_avg_p_c_rec(self, dataloader):
        ...
        for x in dataloader:
            ...
            true_log_p_c_rec = -torch.nn.functional.binary_cross_entropy(p_c_rec, b_c_btrc, reduction='none')  # batch, task, rule, concept
    ...
    def training_step(self, batch, batch_idx):
        ...
        true_log_p_y = -torch.nn.functional.binary_cross_entropy(p_y, b_y_btr, reduction='none')           # batch, task, rule
        if not self.c_pred_in_rec:
            ...
            true_log_p_c_rec = -torch.nn.functional.binary_cross_entropy(p_c_rec, c_pred_btrc, reduction='none')  # batch, task, rule, concept
        true_log_p_c = -torch.nn.functional.binary_cross_entropy(p_c, batch_c, reduction='none')           # batch, concept
...
# Alias to maintain exact interface compatibility with original scripts
MNISTModel = CMRModel
